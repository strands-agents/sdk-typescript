...
import type { HookCallback, HookProvider, StreamEventConstructor, HookCleanup } from './types.js'

/**
 * Represents a callback entry with its source provider.
 */
type CallbackEntry = {
  callback: HookCallback<StreamEvent>
...
export interface HookRegistry {
   ...
   */
  addCallback<T extends StreamEvent>(eventType: StreamEventConstructor<T>, callback: HookCallback<T>): HookCleanup
...
export class HookRegistryImplementation implements HookRegistry {
  private readonly _callbacks: Map<StreamEventConstructor, CallbackEntry[]>
   ...
   */
  addCallback<T extends StreamEvent>(eventType: StreamEventConstructor<T>, callback: HookCallback<T>): HookCleanup {
    const entry: CallbackEntry = { callback: callback as HookCallback<StreamEvent>, source: this._currentProvider }
   ...
   */
  async invokeCallbacks<T extends StreamEvent>(event: T): Promise<T> {
   ...
   */
  private getCallbacksFor<T extends StreamEvent>(event: T): HookCallback<T>[] {
    const entries = this._callbacks.get(event.constructor as StreamEventConstructor<T>) ?? []
