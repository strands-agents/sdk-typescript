...
describe('generateInterruptId', () => {
  it('generates deterministic IDs', () => {
    const id1 = generateInterruptId('tool-1', 'approval')
    const id2 = generateInterruptId('tool-1', 'approval')
    expect(id1).toBe(id2)
  })

  it('generates different IDs for different names', () => {
    const id1 = generateInterruptId('tool-1', 'approval')
    const id2 = generateInterruptId('tool-1', 'rejection')
    expect(id1).not.toBe(id2)
  })

  it('generates different IDs for different tool use IDs', () => {
    const id1 = generateInterruptId('tool-1', 'approval')
    const id2 = generateInterruptId('tool-2', 'approval')
    expect(id1).not.toBe(id2)
  })

  it('includes version prefix', () => {
    const id = generateInterruptId('tool-1', 'approval')
