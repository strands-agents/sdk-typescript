...
describe('UserState', () => {
  describe('constructor', () => {
    ...
    it('creates state with initial values', () => {
      const state = new UserState({ key1: 'value1', key2: 42 })
    ...
    it('stores deep copy of initial state', () => {
      ...
      const state = new UserState(initial)
    ...
    it('throws error for function in initial state', () => {
      ...
      expect(() => new UserState(invalidState as never)).toThrow(
  ...
  describe('get', () => {
    ...
    it('returns value when key exists', () => {
      const state = new UserState({ key1: 'value1' })
      expect(state.get('key1')).toBe('value1')
    })

    it('returns deep copy that cannot mutate stored state', () => {
      const state = new UserState({ nested: { value: 'test' } })
    ...
    it('infers correct type with generic state interface', () => {
      ...
      const state = new UserState({ user: { name: 'John', age: 30 }, count: 5, items: ['a', 'b'] })
    ...
    it('returns undefined for non-existent key with typed interface', () => {
      ...
      const state = new UserState({ existing: 'value' })
  ...
  describe('set', () => {
    ...
    it('overwrites existing value', () => {
      const state = new UserState({ key1: 'old' })
      state.set('key1', 'new')
  ...
  describe('delete', () => {
    it('removes existing key', () => {
      const state = new UserState({ key1: 'value1', key2: 'value2' })
      state.delete('key1')
    ...
    it('supports typed usage with generic state interface', () => {
      ...
      const state = new UserState({ user: { name: 'Alice' }, count: 5 })
  ...
  describe('toJSON', () => {
    it('returns deep copy of state', () => {
      const state = new UserState({ key1: 'value1', nested: { deep: true } })
  ...
  describe('loadStateFromJson', () => {
    it('replaces state with json data', () => {
      const state = new UserState({ old: 'data' })
    ...
    it('clears state when given non-object', () => {
      const state = new UserState({ key: 'value' })
