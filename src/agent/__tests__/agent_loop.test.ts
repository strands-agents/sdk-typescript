...
describe('agent_loop', () => {
  ...
  describe('when handling multiple tool uses in sequence', () => {
    it('executes all tools sequentially', async () => {
      ...
      const { result } = await collectGenerator(agent_loop(messages, registry, undefined, provider))
  ...
  describe('when verifying event streaming', () => {
    it('yields all events in correct order', async () => {
      ...
      const { items } = await collectGenerator(agent_loop(messages, registry, undefined, provider))
