...
describe('BedrockModel', () => {
  ...
  describe('streamAggregated', () => {
    ...
    describe('when streaming tool use', () => {
      it('yields complete tool use block with parsed JSON input', async () => {
        setupMockSend(async function* () {
          yield { messageStart: { role: 'assistant' } }
          yield {
            contentBlockStart: {
              contentBlockIndex: 0,
              start: { toolUse: { name: 'calculator', toolUseId: 'tool-123' } },
            },
          }
          yield { contentBlockDelta: { delta: { toolUse: { input: '{"a":' } }, contentBlockIndex: 0 } }
          yield { contentBlockDelta: { delta: { toolUse: { input: '5,"b":3}' } }, contentBlockIndex: 0 } }
          yield { contentBlockStop: { contentBlockIndex: 0 } }
          yield { messageStop: { stopReason: 'tool_use' } }
          yield { metadata: { usage: { inputTokens: 10, outputTokens: 5, totalTokens: 15 } } }
        })
    ...
    describe('when streaming reasoning content', () => {
      it('yields complete reasoning block with all fields', async () => {
        setupMockSend(async function* () {
          yield { messageStart: { role: 'assistant' } }
          yield { contentBlockStart: { contentBlockIndex: 0 } }
          yield { contentBlockDelta: { delta: { reasoningContent: { text: 'Thinking...' } }, contentBlockIndex: 0 } }
          yield { contentBlockDelta: { delta: { reasoningContent: { signature: 'sig123' } }, contentBlockIndex: 0 } }
          yield {
            contentBlockDelta: {
    ...
    describe('when streaming mixed content blocks', () => {
      it('yields text, tool use, and reasoning blocks in correct order', async () => {
        setupMockSend(async function* () {
          yield { messageStart: { role: 'assistant' } }
          // Text block
          yield { contentBlockStart: { contentBlockIndex: 0 } }
          yield { contentBlockDelta: { delta: { text: 'Let me calculate' }, contentBlockIndex: 0 } }
          yield { contentBlockStop: { contentBlockIndex: 0 } }
          // Tool use block
          yield { contentBlockStart: { contentBlockIndex: 1, start: { toolUse: { name: 'calc', toolUseId: 'id-1' } } } }
          yield { contentBlockDelta: { delta: { toolUse: { input: '{"x":10}' } }, contentBlockIndex: 1 } }
          yield { contentBlockStop: { contentBlockIndex: 1 } }
          // Reasoning block
          yield { contentBlockStart: { contentBlockIndex: 2 } }
          yield { contentBlockDelta: { delta: { reasoningContent: { text: 'Done' } }, contentBlockIndex: 2 } }
        ...
        expect(message).toEqual({
          ...
          content: [
            ...
            { type: 'toolUseBlock', name: 'calc', toolUseId: 'id-1', input: { x: 10 } },
            { type: 'reasoningBlock', text: 'Done' },
          ],
        })
      })
    })

    describe('when streaming empty blocks', () => {
      it('yields empty text block when no deltas', async () => {
        setupMockSend(async function* () {
          yield { messageStart: { role: 'assistant' } }
          yield { contentBlockStart: { contentBlockIndex: 0 } }
          yield { contentBlockStop: { contentBlockIndex: 0 } }
