...
import { createMockContext, collectStreamEventsAndResult, extractEventData } from './test-helpers.js'
  ...
  describe('invoke() method', () => {
    ...
    describe('validation', () => {
      ...
      it('validates required fields', async () => {
        ...
        await expect(myTool.invoke({ name: 'John' } as any)).rejects.toThrow()
        await expect(myTool.invoke({ email: 'invalid-email' } as any)).rejects.toThrow()
      })
    })
  ...
  describe('stream() method', () => {
    describe('basic return types', () => {
      it('streams synchronous callback result', async () => {
        ...
        const { events, result } = await collectStreamEventsAndResult(myTool.stream(context))
    ...
    describe('error handling', () => {
      ...
      it('catches async callback errors', async () => {
        ...
        const firstContent = result.content[0]
        if (firstContent && firstContent.type === 'toolResultTextContent') {
          expect(firstContent.text).toBe('Async error')
        }
      })
    })
  })
