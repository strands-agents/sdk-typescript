...
export abstract class ShellSandbox extends Sandbox {
  ...
  async readFile(path: string): Promise<Uint8Array> {
    ...
    const binary = atob(result.stdout.replace(/\s/g, ''))
  ...
  async writeFile(path: string, content: Uint8Array): Promise<void> {
    const encoded = btoa(Array.from(content, (byte) => String.fromCharCode(byte)).join(''))
  ...
  async listFiles(path: string): Promise<FileInfo[]> {
    const quoted = shellQuote(path)
    const result = await this.execute(`test -d ${quoted} || exit 1; ls -1aF ${quoted}`)
    if (result.exitCode !== 0) {
