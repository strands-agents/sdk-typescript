...
describe('InterventionRegistry', () => {
  ...
  describe('constructor', () => {
    ...
    it('invokes observeAgent on handlers that implement LifecycleObserver', () => {
      const seen: LocalAgent[] = []

      class Observer extends InterventionHandler implements LifecycleObserver {
        readonly name = 'observer'
        observeAgent(localAgent: LocalAgent): void {
          seen.push(localAgent)
        }
      }

      new InterventionRegistry([new Observer()], agent)
