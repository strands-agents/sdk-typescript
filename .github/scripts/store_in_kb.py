"""Amazon Bedrock Knowledge Base storage tool for Strands Agents.

This module provides asynchronous content storage capabilities for Amazon Bedrock
Knowledge Bases, enabling agents to persist information for future retrieval and
context enhancement. Supports real-time ingestion with background processing.

Key Features:
1. Asynchronous background storage - non-blocking operation
2. Real-time document ingestion into Bedrock Knowledge Bases
3. Automatic document ID generation with timestamps
4. Metadata packaging with titles and content organization
5. CUSTOM data source support for inline content
6. Comprehensive error handling and logging
7. Environment variable configuration support
8. Thread-safe background processing

How It Works:
------------
1. **Content Validation**: Validates input content and parameters
2. **Background Processing**: Spawns daemon thread for KB operations
3. **Document Packaging**: Wraps content with metadata and timestamps
4. **KB Ingestion**: Uses Bedrock Agent API for real-time ingestion
5. **Immediate Return**: Returns success status without blocking

Common Use Cases:
---------------
- Session context preservation across agent interactions
- Knowledge accumulation for improved future responses
- Research findings and insights storage
- Conversation history and learned patterns
- Important information persistence for team collaboration
...
agent = Agent(tools=[store_in_kb])

# Store research findings with custom title
result = agent.tool.store_in_kb(
    content="Important research findings about AI safety protocols...",
    title="AI Safety Research - Q4 2024",
    knowledge_base_id="KB123ABC"
)

# Store conversation insights with auto-generated title
result = agent.tool.store_in_kb(
    content="User prefers detailed technical explanations with code examples"
)

# Store project context using environment KB
result = agent.tool.store_in_kb(
    content="Project specifications and requirements for the new feature...",
    title="Project Requirements - Feature X"
)
```

Environment Variables:
--------------------
- STRANDS_KNOWLEDGE_BASE_ID: Default knowledge base ID for storage
- AWS_REGION: AWS region for Bedrock service (default: us-west-2)
- AWS credentials: Standard AWS authentication environment variables
"""
