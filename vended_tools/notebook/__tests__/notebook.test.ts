...
describe('notebook tool', () => {
  ...
  const createFreshContext = (): { state: { notebooks: NotebookState['notebooks'] }; context: ToolContext } => {
  ...
  describe('create oper ation', () => {
    it('creates an empty notebook with default name', async () => {
      ...
      expect(state.notebooks.default).toBe('')
    })
    ...
    it('creates an empty notebook with custom name', async () => {
      ...
      expect(state.notebooks.notes).toBe('')
    })
  ...
  describe('list operation', () => {
    ...
    it('lists multiple notebooks with line counts', async () => {
      const { state, context } = createFreshContext()
      state.notebooks.default = ''
      state.notebooks.notes = 'Line 1\nLine 2\nLine 3'
      state.notebooks.todo = 'Single line'
  ...
  describe('read operation', () => {
    it('reads entire notebook with default name', async () => {
      const { state, context } = createFreshContext()
      state.notebooks.default = 'Line 1\nLine 2\nLine 3\nLine 4\nLine 5'
    ...
    it('reads entire notebook with custom name', async () => {
      const { state, context } = createFreshContext()
      state.notebooks.notes = 'Content here'
  ...
  describe('write operation - string replacement', () => {
    it('replaces text in default notebook', async () => {
      const { state, context } = createFreshContext()
      state.notebooks.default = '# Todo List\n\n[ ] Task 1\n[ ] Task 2\n[x] Task 3'
      ...
      expect(state.notebooks.default).toBe('# Todo List\n\n[x] Task 1\n[ ] Task 2\n[x] Task 3')
    })

    it('replaces text in custom notebook', async () => {
      const { state, context } = createFreshContext()
      state.notebooks.notes = 'Original text'
      const result = await notebook.invoke(
        {
          mode: 'write',
          name: 'notes',
      ...
      expect(state.notebooks.notes).toBe('Updated text')
    })
  ...
  describe('write operation - line insertion', () => {
    it('inserts after line number', async () => {
      ...
      expect(state.notebooks.default).toBe('Line 1\nLine 2\nInserted line\nLine 3')
    })
    ...
    it('inserts at beginning (after line 0)', async () => {
      ...
      expect(state.notebooks.default).toBe('First line\nLine 1\nLine 2\nLine 3')
    })
    ...
    it('inserts into custom notebook', async () => {
      ...
      expect(state.notebooks.notes).toBe('First\nMiddle\nSecond')
    })
  })
  ...
  describe('clear operation', () => {
    it('clears default notebook', async () => {
      const { state, context } = createFreshContext()
      state.notebooks.default = 'Some content'
    ...
    it('clears custom notebook', async () => {
      const { state, context } = createFreshContext()
      state.notebooks.notes = 'More content'
    ...
    it('clearing does not affect other notebooks', async () => {
      ...
      expect(state.notebooks.default).toBe('Some content')
    })
  })
  ...
  describe('state persistence', () => {
    it('persists notebooks across operations', async () => {
      ...
      expect(state.notebooks.notes).toBe('Initial')
